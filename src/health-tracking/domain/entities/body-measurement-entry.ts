...
export class BodyMeasurementEntry {
  ...
  static create(input: Input): BodyMeasurementEntry {
    return BodyMeasurementEntry.build(input);
  }
  correct(
    input: Omit<Input, 'correctedFromId' | 'adultProfileId'> & { id: string },
  ): BodyMeasurementEntry {
    return BodyMeasurementEntry.build({
      ...input,
      adultProfileId: this.props.adultProfileId,
      correctedFromId: this.props.id,
      customMeasurementName: input.customMeasurementName ?? this.props.customMeasurementName,
  ...
  private static build(input: Input): BodyMeasurementEntry {
    ...
    if (type.value === 'CUSTOM' && !input.customMeasurementName?.trim())
      throw new InvalidHealthTrackingValueError('Custom measurement name is required.');
    if (type.value !== 'CUSTOM' && input.customMeasurementName)
      throw new InvalidHealthTrackingValueError('Standard measurements cannot have a custom name.');
    const unit = MeasurementUnit.from(input.unit);
    if (!unit.isLength())
      throw new InvalidHealthTrackingValueError('Body measurement requires a length unit.');
    ...
    return new BodyMeasurementEntry({
      id: new BodyMeasurementEntryId(input.id).value,
      adultProfileId: input.adultProfileId,
      type: type.value,
      customMeasurementName: input.customMeasurementName?.trim() || null,
  ...
  toProps(): BodyMeasurementEntryProps {
    return {
      ...this.props,
      value: new Decimal(this.props.value),
      recordedAt: new Date(this.props.recordedAt),
    };
  }
  get id(): string {
  ...
  get value(): Decimal {
    return new Decimal(this.props.value);
  }
