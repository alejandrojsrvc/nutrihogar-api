...
export class BodyWeightEntry {
  ...
  static create(input: Input): BodyWeightEntry {
    return BodyWeightEntry.build(input);
  }
  correct(
    input: Omit<Input, 'correctedFromId' | 'adultProfileId'> & { id: string },
  ): BodyWeightEntry {
    return BodyWeightEntry.build({
      ...input,
  ...
  private static build(input: Input): BodyWeightEntry {
    ...
    if (!unit.isWeight())
      throw new InvalidHealthTrackingValueError('Body weight requires a weight unit.');
