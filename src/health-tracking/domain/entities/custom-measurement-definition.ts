...
export class CustomMeasurementDefinition {
  ...
  constructor(input: { name: string; unit: 'CM' | 'IN'; enabled?: boolean }) {
    ...
    if (name.length > 100) throw new InvalidHealthTrackingValueError('Custom measurement name is too long.');
    MeasurementUnit.from(input.unit);
    this.name = name;
    this.normalizedName = name.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLocaleLowerCase();
    this.unit = input.unit;
    this.enabled = input.enabled ?? true;
  }
  toProps(): CustomMeasurementDefinitionProps { return { name: this.name, normalizedName: this.normalizedName, unit: this.unit, enabled: this.enabled }; }
}
