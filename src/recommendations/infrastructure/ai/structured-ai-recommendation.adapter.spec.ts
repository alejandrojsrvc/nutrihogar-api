...
describe('StructuredAiRecommendationAdapter', () => {
  it('forwards operation, model and bounded execution options', async () => {
    const complete = jest.fn().mockResolvedValue({
      schemaVersion: 'v1',
      payload: { items: [] },
      metadata: { provider: 'fake', model: 'weekly-v1' },
    });
...
function context(): WeeklyPlanGenerationContext {
  return { schemaVersion: 'v1', contextVersion: 'context-v1' };
}
